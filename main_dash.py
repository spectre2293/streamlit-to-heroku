...
select = st.sidebar.selectbox("Use dropdown arrow to navigate",
                              ('Welcome', 'About Project', 'NFT Price Predictor', 'NFT Comparison'))

if select == 'Welcome':
    st.title("NFT Price Tracker")
    st.header(
        "Welcome to the Metajeune NFT Price Tracker Dashboard!Feel free to use the dropdown menu on the left to navigate this app")
    st.image("nft.jpg")
...
if select == 'About Project':
    ...
    st.write(
        '- A digital asset that is unique and represents real-world tangible objects like art, music, in-game items, videos, audio recording and photos.')
    ...
    st.write(
        "- NFTs have skyrocketed in popularity over the last year; 3,200 active collections in OpenSea at the end of 2021; 193 at the beginning of March 2020")
    st.write(
        "- NFTs are an important element of the Metaverse; It is estimated that bringing the Metaverse to life may represent $1 Trillion USD in annual revenues; It is expected that Gen Z will be the main driver of early adoption of Web 3.0/Metaverse.")
    st.write(
        "- NFTs are retail-driven; It is quite concentrated with the top 95 collections representing half of all NFT flips on OpenSea.")

    st.subheader("The NFT Price Tracker")
    st.write(
        "Meet our typical user, John: A young professional who has $10 000 CAD to invest in the new emerging volatile market of NFTs. However, John knows very little about NFTs and even less about where to begin. His preliminary research has not led him very far.")
    st.write(
        "There is a need for: An interactive platform designed for the novice investor, trader or collector to understand the NFT market; Providing information on trends, sales, recommendations on when, how and if to purchase or sell new and established collections ")
    st.write("The NFT Price Tracker will:")
    st.write(
        "- Track and predict future NFT prices for 30 days based on average prices for 5 popular NFT collections over  6-months. ")
    ...
    st.write(
        "- Identify  top NFT projects based on popularity and activity, to help new NFT investors and traders decide which collections to purchase based on their risk tolerance.")
    ...
    st.write(
        f" - Data Source: NFT floor prices were pulled from various top NFT collections using the collection ‘slug’.Data sourced from OpenSea API.")
    st.write(" - Tool Used: APIs Python Monte Carlo simulation")
    ...
    st.write(
        "Challenge: A Metamask wallet with non-zero Eth balance was required. Accessing the data from OpenSea. Mitigants: Submitted three different API requests to OpenSea early in the process to minimize risk of not receiving the key. Ensured an active metamask wallet with Eth was available.")
    st.write(
        "Challenge: Insufficient data from some NFT collections. Mitigants: Collections within the top 25 most popular NFTs with the longest price history available were used.")
    st.write(
        "Challenge: Some collections have significant price volatility which impacts the Monte Carlo Simulations. Mitigants: Manual adjustments and data cleanup were required.Used average daily price to show trends over time.Used rolling median  to show Monte carlo Simulations")
    st.write(
        "Challenge: Making the data accessible to the end user. Mitigants: Stremlit to display charts and data in a user friendly and interactive manner.")
    ...
    st.write(
        "- Compare different NFT collections: User will be able to input a specific collection and/or select from a list to obtain statistics.")
...
if select == 'NFT Price Predictor':
    st.title(select)
    nft_select = st.selectbox("Select an NFT Project", (
    'Bored Ape Yacht Club', 'Mutant Ape Yacht Club', 'Doodles', 'Crypto Kitties', 'Pudgy Penguins'))
    if nft_select == 'Bored Ape Yacht Club':
        ...
        st.write("")
        st.write(
            f"- There is a 95% probability that the price of this NFT collection will range between a 34% decrease and 91% increase.")
    ...
    if nft_select == 'Doodles':
        ...
        st.write(
            f"- There is a 95% probability that the price of this NFT collection will range between a 28% decrease and 249% increase.")
    ...
    if nft_select == 'Crypto Kitties':
        ...
        st.write(
            f"- There is a 95% probability that the price of this NFT collection will range between 0 and 74% increase.")
        # st.subheader("Distribution Plot")
    ...
    if nft_select == 'Mutant Ape Yacht Club':
        ...
        st.write(
            f"- There is a 95% probability that the price of this NFT collection will range between a 28% decrease and 126% increase.")
    ...
    if nft_select == 'Pudgy Penguins':
        ...
        st.write(
            f"- There is a 95% probability that the price of this NFT collection will range between a 86% decrease and 225% increase.")
...
if select == 'NFT Comparison':
    ...
    with col1:
        nft_select = st.selectbox("Select Project One", (
        'Bored Ape Yacht Club', 'Mutant Ape Yacht Club', 'Doodles', 'Crypto Kitties', 'Pudgy Penguins'))
        if nft_select == 'Bored Ape Yacht Club':
            baycdf = pd.read_csv('BAYCclose.csv')
            baycdf = baycdf.set_index('event_date')
    ...
    with col2:
        nft_select2 = st.selectbox("Select Project Two", (
        'Bored Ape Yacht Club', 'Mutant Ape Yacht Club', 'Doodles', 'Crypto Kitties', 'Pudgy Penguins'))
